...
public class Adfinder {
    ...
    public void log(String message) {
        try {
            BufferedWriter write = new BufferedWriter(new FileWriter("plugins/CCChat/Log.txt", true));
            write.append(message);
            write.newLine();
            write.flush();
            write.close();
        } catch (IOException ex) {
            plugin.getLogger().log(Level.WARNING, "CCChat error while saving message on the log file msg:" + ex.getMessage());
        }
    }
    ...
    private void sendWarning(Player player, String message, int type, int where) {
        ...
        Bukkit.getServer().getLogger().info("[CCChat] " + player.getDisplayName() + " was logged for " + typeToX(type, 2) + " in " + whereToTXT(where) + ".");
        ...
        if (warn.get(player) < 2) {
            ...
            player.sendMessage(ChatColor.DARK_GREEN + "[CCChat]" + " " + ChatColor.RED + typeToX(type, 3));
        } else {
    ...
    private void takeAction(Player player, int type) {
        ...
        String broadcastMessage = ChatColor.DARK_GREEN + "[CCChat] " + ChatColor.RED + player.getDisplayName() + ChatColor.DARK_GREEN + " has been " + getActionType(command) + " for " + typeToX(type, 2);
    ...
    public void loadWhitelist() {
        ...
        try {
            BufferedReader read = new BufferedReader(new FileReader("plugins/CCChat/Whitelist.txt"));
