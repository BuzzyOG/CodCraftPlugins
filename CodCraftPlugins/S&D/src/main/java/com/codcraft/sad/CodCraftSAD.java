...
public class CodCraftSAD extends CCGamePlugin {
	...
	public Map<String, SADModel> currentmap = new HashMap<>();
	...
	private void team2Load() {
	    File spawns = new File("./plugins/SAD/team2.yml");
	...
	private void team1Load() {
	     File spawns = new File("./plugins/SAD/team1.yml");
	     YamlConfiguration config = YamlConfiguration.loadConfiguration(spawns);
		    for(String s : config.getConfigurationSection("maps").getKeys(false)) {
	...
	private void spawnLoad() {
		     File spawns = new File("./plugins/SAD/config.yml");
		     YamlConfiguration config = YamlConfiguration.loadConfiguration(spawns);
		     for(String s : config.getConfigurationSection("maps").getKeys(false)) {
	...
	public void GameTimer() {
		...
		Bukkit.getScheduler().runTaskTimer(this, new Runnable() {
			...
			public void run() {
				...
				for(Entry<String, SADModel> model : currentmap.entrySet()) {
					if(model.getValue().state == null) {
						model.getValue().state = com.codcraft.sad.CodCraftSAD.GameState.LOBBY;
						model.getValue().gametime = 30;
					}
					Random rnd = new Random();
					if(model.getValue().state == com.codcraft.sad.CodCraftSAD.GameState.LOBBY) {
						...
						if(model.getValue().gametime > 0) {
							...
							model.getValue().state = com.codcraft.sad.CodCraftSAD.GameState.INGAME;
					...
					if(model.getValue().state == com.codcraft.sad.CodCraftSAD.GameState.INGAME) {
						if(model.getValue().gametime > 0) {
							model.getValue().gametime--;
						} else {
